...
class AttentionModel():
    def __init__(self, dim, maxlen, indexes):
        ...
        self.feature_dim_dict = {"sparse": [iFeature, pFeature, aFeature],
                                 "dense": [SingleFeat('price', False)]}

        self.behavior_feature_list = ["item", "position", "action"]
    ...
    def generate_data(self, df, mode="train"):
        ...
        sessions, itemIds, cand_items, cand_actions, cand_positions, cand_prices, seq_items, seq_actions, seq_prices, seq_positions, labels = [], [], [], [], [], [], [], [], [], [], []
        sessionLens = []
        sId = 0
        for idx, rows in df.groupby("session_id"):
            ...
            if mode == "train":
                ...
                action, position, price = self.get_features(gtItem, impressions, prices, lastRow)
            ...
            if mode == "train":
                ...
                labels.append(1)
                sessionLens.append(1 if len(seq_item) > 0 else 0 )
        ...
        sessionLens = np.array(sessionLens)

        feature_dict = {'item': cand_items, 'position': cand_positions, 'action': cand_actions,
                        'price': cand_prices,
                        'seq_item': seq_items, 'seq_position': seq_positions,
